...
export async function uploadFileForInfo(chunks,chunkSizes)
{
    try {
        ...
        let res = await fetch("http://localhost:3003/upload/finisheduploadffprob", {method:"POST", body:JSON.stringify({oldFileName: chunks.name.split(".")[0], ext: chunks.name.split(".")[1],chunks:chunks.mediaChunks.length - 1, chunkSizes}), headers: {'Content-Type': 'application/json'}})
        if(res.ok)
        {
